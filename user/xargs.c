    ...
    while (read(0, &c, sizeof(c))) {
        if (c != '\n') {
            ...
            if (fork() == 0) { //为什么要用fork
                ...
                for (int k = 1; k < argc; k++) // 把命令行参数保存到参数数组中 比如 (echo 1 ; echo 2) | xargs -n 1 echo
                {
                ...
                exec(argv[1], commands); // 通常用于在一个进程中加载并执行另一个程序。它会用新的程序替代当前进程的镜像，即在执行新程序时，当前进程的代码和数据都会被新程序替代。
            } else {
                wait(0);
                i = 0;
